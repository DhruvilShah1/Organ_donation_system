...
<body>
  ...
  $row = mysqli_fetch_assoc($result);
    ...
    ?>
            ...
            <input type="text" id="name" name="d_name"  value ="<?php  echo $row['d_name']; ?>"required>

            <label for="phone">Phone Number:</label>
            <input type="text" id="phone" name="d_phone" pattern="^\d{10}$" value ="<?php  echo $row['d_phone']; ?>"required>
            <small>Enter 10-digit phone number (without spaces).</small>

            <label for="email">Email Address:</label>
            <input type="email" id="email" name="d_email" value ="<?php  echo $row['d_email']; ?>"required>

            <label for="address">Address:</label>
            <textarea id="address" name="d_address" rows="4" required><?php  echo $row['d_address']; ?></textarea>

            <div class="field">
    <label for="donate_organ">Need Of Organ:</label>
    <div class="checkbox-group">
        <label for="donate_heart">
            <input type="checkbox" id="donate_heart" name="d_organ[]" value="heart" <?php if(in_array("heart", $r_organ1)){ echo "checked"; }?>>
            Heart
        </label>
        <label for="donate_lungs">
            <input type="checkbox" id="donate_lungs" name="d_organ[]" value="lungs" <?php if(in_array("lungs", $r_organ1)){ echo "checked";} ?>>
            Lungs
        </label>
        <label for="donate_kidneys">
            <input type="checkbox" id="donate_kidneys" name="d_organ[]" value="kidneys" <?php if(in_array("kidneys", $r_organ1)){ echo "checked";} ?>>
            Kidneys
        </label>
        <label for="other">
  <input type="checkbox" id="other" name="d_organ[]"> Other
  </label>
  ...
  <br>
            ...
            <input type="text" id="city" name="d_city" value ="<?php  echo $row['d_city']; ?>"required>
        </fieldset>
...
include("connection.php");
  ...
  if(isset($_POST['update'])){
     ...
     $phone = $_POST['d_phone'];
     $email = $_POST['d_email'];
     $r_organ = $_POST['d_organ'];
     $selectedOrgans = isset($_POST['d_organ']) ? $_POST['d_organ'] : [];
     ...
     $city= $_POST['d_city'];
     $query = "UPDATE `donor_register` SET d_name='$name', d_phone ='$phone' , d_email='$email',d_address ='$address',d_city = '$city' ,d_organ='$combinedValue' WHERE R_id=$user_id";
     $result=mysqli_query($conn,$query);
  }
?>
    <header>
...
$query = "SELECT * FROM `donor_login` as rl, `donor_register` as rr WHERE rl.id = $user_id AND rr.R_id = $user_id ";
$result = mysqli_query($conn, $query);
...
?>
        <main>
            <section class="photo-info">
                ...
                ?>
                <div class="details">
                    <p><span class="label">Name:</span> <?php echo $row['d_name']; ?></p>
                    <p><span class="label">Email:</span> <?php echo $row['d_email']; ?> </p>
                    <p><span class="label">City:</span> <?php echo $row['d_city']; ?></p>
                    <p><span class="label">Phone:</span> <?php echo $row['d_phone']; ?></p>
                    <p><span class="label">Address:</span> <?php echo $row['d_address']; ?></p>
                </div>
            </section>
            ...
            <section class="donations">
                ...
                <ul>
                    <li>
                        <span class="label">Organs:</span> <?php echo $row['d_organ']; ?>
                    </li>
                    <li>
